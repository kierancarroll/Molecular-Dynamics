...
CONF_GRO="$PARENT_DIR/conf246.gro"
TOP_FILE="$PARENT_DIR/1vet.top"
INDEX_FILE="$PARENT_DIR/index.ndx"

# Short equilibration
gmx grompp -f "$NPT_MDP" -c "$CONF_GRO" -r "$CONF_GRO" -p "$TOP_FILE" -n "$INDEX_FILE" -o npt246.tpr -maxwarn 1
if [ $? -ne 0 ]; then
    echo "FAIL: grompp equilibration failed" > FAIL
    exit 1
fi

gmx mdrun -deffnm npt246 -nt 1 -v
if [ $? -ne 0 ]; then
...
# Umbrella run
gmx grompp -f "$MD_MDP" -c npt246.gro -r npt246.gro -t npt246.cpt -p "$TOP_FILE" -n "$INDEX_FILE" -o umbrella246.tpr -maxwarn 1
if [ $? -ne 0 ]; then
    echo "FAIL: grompp umbrella failed" > FAIL
    exit 1
fi

gmx mdrun -deffnm umbrella246 -nt 1 -v
if [ $? -ne 0 ]; then
